...
function ApplicantCard({ entry, onStatusChange }) {
    ...
    return (
        ...
        >
            ...
            >
                {/* Avatar */}
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-purple-600 to-pink-500 flex items-center justify-center text-white font-black text-base flex-shrink-0">
                    {(entry.fullName || '?')[0].toUpperCase()}
                </div>

                {/* Name + email */}
                <div className="flex-1 min-w-0">
                    <p className="text-white font-bold text-sm truncate">{entry.fullName || '—'}</p>
            ...
            <AnimatePresence>
                {expanded && (
                    ...
                    >
                        <div className="border-t border-purple-500/10 px-5 py-5 space-y-6">
                            {/* Detail grid */}
                            ...
                            {tasksList.length > 0 && (
                                <div>
                                    <p className="text-[11px] uppercase tracking-widest text-gray-500 font-semibold mb-2">Social Tasks Completed</p>
                                    <div className="flex flex-wrap gap-2">
                                        {tasksList.map((t) => (
                                            <span key={t} className="px-3 py-1 rounded-full text-xs font-semibold bg-purple-500/10 text-purple-300 border border-purple-500/20">
                                                {t}
                                            </span>
                                        ))}
                                    </div>
                                    ...
                                    <>
                                        {entry.status !== 'approved' && (
                                            <motion.button
                                                ...
                                                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold hover:bg-emerald-500/20 transition"
                                            >
                                        ...
                                        {entry.status !== 'rejected' && (
                                            <motion.button
                                                ...
                                                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold hover:bg-red-500/20 transition"
                                            >
                                        ...
                                        {(entry.status === 'approved' || entry.status === 'rejected') && (
                                            <motion.button
                                                ...
                                                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-xs font-bold hover:bg-yellow-500/20 transition"
                                            >
        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <p className="text-gray-500 mt-1">
                    {entries.length} {entries.length === 1 ? 'person has' : 'people have'} applied for the giveaway.
                </p>
            </motion.div>
            ...
            >
                ...
                {/* Status tabs */}
                <div className="flex items-center gap-1 bg-[#0d0d18] border border-purple-500/15 rounded-xl p-1">
                    {filters.map((f) => (
                        <button
                            key={f.key}
            ...
            {loading ? (
                ...
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="text-center py-20 bg-[#0d0d18] border border-purple-500/10 rounded-2xl"
                >
